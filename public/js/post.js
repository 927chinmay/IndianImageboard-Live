...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const fetchPostData = async () => {
        try {
            ...
            // Safely handle post author
            const postUser = post.userId;
            const authorName = postUser ? postUser.username : 'Anonymous';
            const authorProfileLink = postUser ? `/profile.html?id=${postUser._id}&username=${authorName}` : '#';

            let deleteBtnHtml = (postUser && loggedInUserId === postUser._id.toString()) ? `<button class="delete-post-btn" data-id="${post._id}">Delete Post</button>` : '';
            ...
            // --- THIS IS THE FULLY RESTORED COMMENT LOGIC ---
